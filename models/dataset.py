...
def find_k_nearest_neighbors(point, k, dataset):
    # 将输入点转换为 PyTorch 张量并添加批处理维度
    point_tensor = torch.tensor(point).unsqueeze(0).float().to(dataset.device)

    # 使用定义的函数查找最近邻点的索引
    nearest_idx = search_nearest_point(point_tensor, dataset.point)

    # 从数据集中获取前 k 个最近邻点
    k_nearest_neighbors = dataset.point[nearest_idx[:k]]
